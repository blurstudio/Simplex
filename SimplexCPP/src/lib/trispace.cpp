...
void TriSpace::triangulate(){
	std::unordered_map<std::vector<int>, std::vector<std::vector<double>> ,vectorHash<int>> d;
	for ( auto fit = floaters.begin(); fit != floaters.end(); ++fit){
		//for (auto f : floaters){
		auto &f = *fit;
...
std::vector<std::vector<int>> TriSpace::pointToAdjSimp(const std::vector<double> &pt, double eps) {
...
void TriSpace::rec(const std::vector<double> &point, const std::vector<int> &oVals, const std::vector<int> &simp, std::vector<std::vector<int> > &out, double eps) const {
	if (point.empty()){
		out.push_back(simp);
		return;
	}
...
std::vector<std::vector<std::vector<double> > > TriSpace::splitSimps(const std::vector<std::vector<double> > &pts, const std::vector<std::vector<int> > &simps) const {
